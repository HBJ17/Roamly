...
def dashboard():
    ...
    cursor.execute('''
        SELECT 
            ...
            COALESCE(p.destination, h.city, CONCAT(t.source_city, ' to ', t.destination_city)) as item_destination,
...
def profile():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('''
            UPDATE users
            SET email = %s, full_name = %s, phone = %s, address = %s, bio = %s
            WHERE id = %s
...
def cancel_booking(booking_id):
    ...
    if booking:
        cursor.execute("UPDATE bookings SET status = 'Cancelled', trip_status = 'Cancelled' WHERE id = %s", (booking_id,))
        conn.commit()
...
def complete_trip(booking_id):
    ...
    if booking:
        cursor.execute("UPDATE bookings SET trip_status = 'Completed' WHERE id = %s", (booking_id,))
        conn.commit()
...
def trip_timeline(booking_id):
    ...
    cursor.execute('SELECT * FROM reviews WHERE user_id = %s AND item_type = %s AND item_id = %s', (user_id, item_type, item_id))
...
def toggle_favorite(item_type, item_id):
    ...
    # check existing
    cursor.execute('SELECT id FROM saved_items WHERE user_id = %s AND item_type = %s AND item_id = %s', (user_id, item_type, item_id))
    existing = cursor.fetchone()

    if existing:
        cursor.execute('DELETE FROM saved_items WHERE id = %s', (existing['id'],))
        flash(f'Removed from your saved {item_type}s.', 'info')
    else:
        cursor.execute('INSERT INTO saved_items (user_id, item_type, item_id) VALUES (%s, %s, %s)', (user_id, item_type, item_id))
